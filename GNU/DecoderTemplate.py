...
while boolean == True:
	...
	index,msg=get_bytes(bin_out,index,4)	
	
	msg=bytes_to_hex(msg)
	
	index,msg=get_bytes(bin_out,index,1)	
	
	msg=bytes_to_hex(msg)

	payload_length_decimal= int(msg, 16)

	index,msg=get_bytes(bin_out, index, payload_length_decimal)

	msg=decode_hex(msg)

	print(msg)

        index,crc=get_bytes(bin_out,index,4)	
